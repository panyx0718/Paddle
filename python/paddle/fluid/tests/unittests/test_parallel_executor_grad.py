...
def squeeze_excitation(input, num_channels, reduction_ratio):
    ...
    squeeze = fluid.layers.fc(input=pool,
                              ...
                              act='relu',
                              name="squeeze_excitation_fc1")
    excitation = fluid.layers.fc(input=squeeze,
                                 size=num_channels,
                                 act='sigmoid',
                                 name="squeeze_excitation_fc2")
    scale = fluid.layers.elementwise_mul(
        x=input,
        y=excitation,
        axis=0,
        name='squeeze_excitation_elementwise_mul')
    return scale
...
def lenet(data, label):
    ...
    """
    fc1 = fluid.layers.fc(pool1, size=500, act='relu')
...
class CompareParallelExecutorAndParallelDo(unittest.TestCase):
    ...
    def parallel_exe(self, seed, train_inputs):
        ...
        with fluid.program_guard(main, startup):
            ...
            for data in train_inputs:
                ret = train_exe.run(fetch_list=[avg_cost.name],
                                    feed=feeder.feed(data))
                loss = np.mean(np.array(ret[0]))
                losses.append(loss)
        return losses

    def single_device(self, seed, train_inputs):
        main = fluid.Program()
        startup = fluid.Program()
        startup.random_seed = seed
        main.random_seed = seed
        ...
        with fluid.unique_name.guard(''), fluid.program_guard(main, startup):
            ...
            for op in main.blocks[0].ops:
                for out_var in op.output_arg_names:
                    fetch_vars.append(str(out_var))
            """
            ...
            # fetch_vars = ['squeeze_excitation_elementwise_mul.tmp_0']
            # fetch_vars=['conv2d_0.w_0', 'conv2d_0.w_0@GRAD']
            for data in train_inputs:
                ret = exe.run(main,
                              fetch_list=[avg_cost.name] + fetch_vars,
                              feed=feeder.feed(data))
    ...
    def test_compare_grad(self):
        ...
        for _ in range(iter):
            ...
            train_inputs.append([(np.reshape(
                np.ones(shape=[3, 224, 224]), [-1]), 2), (np.reshape(
                    np.ones(shape=[3, 224, 224]), [-1]), 2)])
            # train_inputs.append(
            #     [(np.reshape(np.random.rand(3, 224, 224), [-1]), 0),
            #      (np.reshape(np.random.rand(3, 224, 224), [-1]), 0)])

        single_losses = self.single_device(seed, train_inputs)
        sys.stderr.write('\n\n\n')
        exe_losses = self.single_device(seed, train_inputs)

        sys.stderr.write('loss: %s %s\n' % (single_losses, exe_losses))
        for i in range(iter):
            self.assertTrue(
                np.allclose(
                    single_losses[i], exe_losses[i], atol=1e-8),
                "Single loss: " + str(single_losses[i]) + "\n ParallelExe loss:"
                + str(exe_losses[i]))
